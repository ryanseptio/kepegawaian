...
class M_pegawai extends CI_Model {
	...
	public function select_all() {
		$sql = " SELECT pegawai.id AS id, pegawai.nama AS pegawai, pegawai.telp AS telp, kota.nama AS kota, kelamin.nama AS kelamin, jabatan.nama AS jabatan , absensi, username, departemen, npp, status_penilaian FROM pegawai, kota, kelamin, jabatan WHERE pegawai.id_kelamin = kelamin.id AND pegawai.id_jabatan = jabatan.id AND pegawai.id_kota = kota.id ORDER BY  pegawai.nama ASC";
	...
	public function select_by_id($id) {
		$sql = "SELECT pegawai.id AS id_pegawai, pegawai.nama AS nama_pegawai, pegawai.id_kota, pegawai.id_kelamin, pegawai.id_jabatan, pegawai.telp AS telp, kota.nama AS kota, kelamin.nama AS kelamin, jabatan.nama AS jabatan, absensi, username, password, departemen, npp, status_penilaian FROM pegawai, kota, kelamin, jabatan WHERE pegawai.id_kota = kota.id AND pegawai.id_kelamin = kelamin.id AND pegawai.id_jabatan = jabatan.id AND pegawai.id = '{$id}'";
	...
	public function update($data) {
		...
		$sql = "UPDATE pegawai SET nama='" .$data['nama'] ."', npp='" .$data['npp'] ."', telp='" .$data['telp'] ."', id_kota='" .$data['kota'] ."', id_kelamin='" .$data['jk'] ."', id_jabatan='" .$data['jabatan'] ."', username='" .$data['username'] ."', password='{$pass}', departemen='" .$data['departemen'] ."', status_penilaian='" .$data['status_penilaian'] ."' WHERE id='" .$data['id'] ."'";
	...
	public function insert($data) {
		...
		$pass = md5($data['password']);
		$sql = "INSERT INTO pegawai VALUES(default,'" .$data['nama'] ."','" .$data['npp'] ."','" .$data['telp'] ."','" .$data['kota'] ."','" .$data['jk'] ."','" .$data['jabatan'] ."',1 , '', '" .$data['username'] ."', '{$pass}', 'user', '" .$data['departemen'] ."', '" .$data['status_penilaian'] ."')";
